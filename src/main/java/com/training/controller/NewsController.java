...
public class NewsController {
    ...
    public Map<String, Object> list(@RequestParam(required = false) String title,
        ...
        try {
            ...
            if (isAdmin) {
                // 管理员查全部或按userId查
                list = newsService.getListByPage(title, summary, author, userId, null, offset, size);
                total = newsService.countByCondition(title, summary, author, userId, null);
            } else {
                // 企业用户查所有已通过审核的动态，或查自己发布的
                if (userId != null && userId.equals(user.getId())) {
                    list = newsService.getListByPage(title, summary, author, user.getId(), null, offset, size);
                    total = newsService.countByCondition(title, summary, author, user.getId(), null);
                } else {
                    list = newsService.getListByPage(title, summary, author, null, 1, offset, size);
                    total = newsService.countByCondition(title, summary, author, null, 1);
                }
            }
